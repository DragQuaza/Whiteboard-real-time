...
Firebase is initialized from the runtime config served at `/config.js`. If you are deploying your own instance, provide your own Firebase project values through environment variables instead of hardcoding them into the client.
...
- `BACKEND_URL` from `/config.js` when provided
...
If you deploy only the static frontend to Vercel or Firebase Hosting, realtime collaboration still depends on a running Socket.IO backend. In that setup, serve a `BACKEND_URL` value through `/config.js` or another bootstrapping step, and allow the frontend origin through `SOCKET_IO_ALLOWED_ORIGINS`.
